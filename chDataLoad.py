...
while (True):
    if (  (  (dev_id == 'aq1') or (dev_id == 'aq2')  ) and (not None in (deviceDict[dev_id].temperature,
    ...
    if ( (dev_id =='ajaqi_1') and (not None in (deviceDict[dev_id].temperature,
     deviceDict[dev_id].pressure, deviceDict[dev_id].pm10, deviceDict[dev_id].pm25))):
        sql = "INSERT INTO aq_data (dateTime, owner, dev_id, temperature, pressure, pm10, pm25) VALUES (%s, %s, %s, %s, %s, %s, %s)"
        vals = (deviceDict[dev_id].now, deviceDict[dev_id].owner, dev_id, deviceDict[dev_id].temperature, deviceDict[dev_id].pressure,
         deviceDict[dev_id].pm10, deviceDict[dev_id].pm25)
        mycursor.execute(sql, vals)
        mydb.commit()
        time.sleep(1)
        print("Inserted an aj record")
        deviceDict[dev_id].temperature = deviceDict[dev_id].humidity = deviceDict[dev_id].pressure = deviceDict[dev_id].pm10 = deviceDict[dev_id].pm25 = deviceDict[dev_id].now = None
